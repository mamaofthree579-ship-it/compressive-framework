...
st.title("CGUP Inference with real data")
...
try:
    ...
    st.write("Using real strain.")
except Exception as e:
    st.warning(f"Fetch failed ({e}); falling back to PSD noise.")
    ifos = InterferometerList(['H1','L1'])
...
if st.button("Run CGUP real"):
    with tempfile.TemporaryDirectory() as outdir:
        try:
            res = bilby.run_sampler(likelihood=like, priors=priors,
                                    sampler='dynesty', nlive=200,
                                    outdir=outdir, label='cgup_real', verbose=False)
            st.pyplot(res.plot_corner(['mass_1','mass_2','alpha','lam']))
