...
fs = st.sidebar.slider("Sampling rate (Hz)", 1,500,100) # user sets guess
...
if eeg is not None:
    ...
    st.write("Dominant freq:",dom); psi_b=wave(dom,1.0,gamma)
else: psi_b=wave(38.0,1.0,gamma)

psi_h=wave(1.1,0.5,gamma*0.5); psi_g=wave(0.12,0.2,gamma*0.2)
...
t_ent=np.arange(len(ent))*win/fs
