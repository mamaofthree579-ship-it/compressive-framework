...
st.title("🧠 Quantum Double-Slit Observer Simulator (3D Enhanced)")
st.markdown("""
Explore how **observation** — whether by a detector, instrument, or human — affects the **quantum interference pattern**
of the classic **double-slit experiment**.  
Now with **3D field visualization** to see |ψ|² dynamically evolve in real space.
""")
...
def observer_field(X, Y, observer_type, strength, t, params=None):
    ...
    if observer_type == "detector":
        detector_x, detector_sigma, detector_k = 0, params.get("detector_sigma", 1.0), 6.0
...
if st.button("🎞️ Export Animation"):
    ...
    gif_path = "/tmp/double_slit_observer_3d.gif"
    imageio.mimsave(gif_path, frames_img, fps=15)
    with open(gif_path, "rb") as f:
        st.download_button("📥 Download 2D Animation (GIF)", f, file_name="double_slit_observer_3d.gif", mime="image/gif")

st.success("✅ Simulation ready. Toggle 3D visualization for field waveforms.")
