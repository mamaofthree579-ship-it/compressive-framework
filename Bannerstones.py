...
st.header("Crescent bannerstone drill sim")
a, b = 7.0, 1.0 # flat crescent profile
...
for r in radii:
    ...
    m = V*density
    I = (1/5)*m*(b**2+c**2) # small b reduces I
...
ax.set_xlabel("Hole radius (cm)"); ax.set_ylabel("Depth per area")
st.pyplot(fig)

st.download_button("Export", pd.DataFrame({"r":radii,"eff":effs}).to_csv(index=False), "crescent.csv")
