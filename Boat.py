...
# Tail parameters (adaptive)
tail_angles = st.sidebar.slider("Tail Feather Angle Range (°)", -30, 30, 15)
...
for env_name, env in environments.items():
    ...
    # Dugout stability (rudderless)
    dugout_stab = 0.5
    # Dugout wave match
    dugout_wave = 1 - abs(dugout["length"] - wavelength/2)/(wavelength/2 + 0.001)
    
    # QH-1 adaptive hull
    qh1["length"] = wavelength/2  # adaptive wave-matched
...
# -------------------------
# Wake Visualization for Ocean
# -------------------------
st.subheader("Wake Decay Comparison (Ocean)")
x_line = np.linspace(0, 20, 500)
dugout_y = environments["Ocean (Rough)"]["amp"] * np.exp(-0.1*x_line) * np.sin(environments["Ocean (Rough)"]["freq"]*x_line)
qh1_y = environments["Ocean (Rough)"]["amp"] * np.exp(-0.3*x_line) * np.sin(environments["Ocean (Rough)"]["freq"]*x_line))
...
st.write("""
### Interpretation:
- Dugout canoes perform well in calm lakes (wave match is close to zero loss).  
- QH-1 maintains perfect wave matching in rivers and ocean, demonstrating adaptive advantage.  
- Stability improves in rougher conditions due to tail spread and modular hull adjustment.  
- Wake decay is faster for QH-1, showing reduced energy loss and smoother sailing.
""")
